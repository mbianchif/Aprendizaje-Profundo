...
Bajo un entrenamiento optimizado por Descenso por Gradiente, la actualización del vector de parámetros global tras un intervalo de sincronización se modela como:

\begin{equation}
W_{t + E} = W_{t} - \frac{\lambda}{N} \sum_{i=1}^{N} G_{i}^{(E)}
\end{equation}

donde $N$ es el número de \emph{workers}, $\lambda$ es la tasa de aprendizaje y $G_{i}^{(E)}$ representa la acumulación de gradientes calculados por el \emph{worker} $i$ a lo largo de las $E$ épocas locales.
